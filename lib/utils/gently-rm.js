...
function isSafeToRm (parent, target, cb) {
  ...
  if (target.managed) {
    log.silly('gentlyRm', target.path +
      (target.path != target.managed[0] ? ' -> ' + target.managed[0] : ''),
      "is under npm's control, in", target.managed[1])
  }
...
function isEverInside (target, paths, cb) {
  ...
  asyncMap(paths, readAllLinks, iferr(skipENOENT, function (resolvedPaths) {
    readAllLinks(target, iferr(skipENOENT, function (targets) {
      var areany = areAnyInsideAny(targets, resolvedPaths)
      cb(null, [areany])
    }))
  }))
}
...
function areAnyInsideAny (targets, paths) {
  ...
  for (var ii = 0; ii < toCheck.length; ++ii) {
    ...
    if (inside && path) return inside && path && [target, path]
  }
  return false
}
...
function resolveSymlink (symlink, cb) {
  ...
  readLinkOrShim(symlink, function (er, symlinkTarget) {
    if (er) return cb(er)
